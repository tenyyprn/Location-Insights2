"""
チャットサービス - Location Insights AI応答生成
"""
import os
import logging
...
現在分析中の住所やコンテキストがある場合は、それを考慮して回答してください。"""
    ...
    async def generate_response(
        ...
        try:
            ...
            # OpenAI APIで応答を生成
            return await self._generate_openai_response(message, session_id, current_context)
...
Location Insights AIでは、以下のような地域情報についてお答えできます：
        ...
        if context.get("lifestyle_scores"):
            ...
            for category, score in scores.get("breakdown", {}).items():
                category_names = {
                    ...
                    "shopping": "買い物", "safety": "安全性", "environment": "環境",
                    "cultural": "文化"
                }
